...
def delete_client(
    ...
    """Eliminación lógica para conservar servicios e historial relacionados."""
    client = db.query(Client).filter(Client.id == client_id, Client.activo == True).first()
    if not client:
        raise HTTPException(status_code=404, detail="Cliente no encontrado")

    # IMPLEMENTACIÓN: se desactiva en lugar de borrar físicamente para no
    # romper las relaciones existentes con equipos, servicios y auditoría.
    client.activo = False
    log_audit(db, usuario_id=current_user.id, accion="ELIMINAR", entidad="Client", entidad_id=client.id, descripcion=f"Cliente eliminado: {client.nombres} {client.apellidos}")
    db.commit()
    return None
