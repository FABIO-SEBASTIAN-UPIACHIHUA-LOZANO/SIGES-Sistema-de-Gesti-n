...
class Invoice(Base):
    ...
    tipo_comprobante: Mapped[InvoiceType] = mapped_column(
        SQLEnum(InvoiceType),
        nullable=False,
        index=True,
    )
    ...
    cliente_documento: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
    )

    cliente_direccion: Mapped[Optional[str]] = mapped_column(
        String(255),
        nullable=True,
    )
    ...
    estado: Mapped[InvoiceStatus] = mapped_column(
        SQLEnum(InvoiceStatus),
        nullable=False,
        default=InvoiceStatus.EMITIDO,
        index=True,
    )
    ...
    detalles: Mapped[list["InvoiceItem"]] = relationship(
        "InvoiceItem",
        back_populates="comprobante",
...
class InvoiceItem(Base):
    ...
    comprobante: Mapped["Invoice"] = relationship(
        "Invoice",
        back_populates="detalles",
    )

    producto: Mapped[Optional["Product"]] = relationship(
        "Product",
    )